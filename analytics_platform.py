...
<div class="header">
  ...
  <div style="display:flex; gap:10px; align-items:center;">
    <a href="/page/leave-kpi" class="refresh-btn" style="background:rgba(17,85,204,0.28);border-color:rgba(255,255,255,0.5);">&#x1F4C5; Leave KPI</a>
...
def build_leave_kpi_data(db, sub_map, filters=None):
    """Build all data needed for the dedicated Leave KPI report."""
    filters = filters or {}
    start_date = filters.get("start_date")
    end_date   = filters.get("end_date")
    selected_sub = filters.get("subsidiary", "All")
    ...
    def _name(r):
        for f in ("name", "staff_name", "employee_name", "applicant_name",
                  "full_name", "employee", "user_name", "staffName", "employeeName"):
            v = _str(r.get(f))
            if v:
                return normalize_cbc_text(v)
        return "Unknown"
    ...
    # Load and filter
    raw = list(db["Leave_Request"].find({}))
    leaves = []
    for r in raw:
        sub = resolve_sub(r.get("subsidiary_id", ""), sub_map)
        if selected_sub != "All" and sub != selected_sub:
            continue
        dt = record_date(r)
        if start_date and dt and dt.date() < start_date:
            continue
        if end_date and dt and dt.date() > end_date:
            continue
        leaves.append(r)
    ...
    for r in leaves:
        ...
        rows.append({
            "staff_name":    _name(r),
            "employee_id":   _str(r.get("employee_id") or r.get("employee_code") or r.get("staffId") or r.get("staff_id") or "—"),
            "department":    normalize_cbc_text(_str(r.get("department") or r.get("dept") or "—")),
            "position":      normalize_cbc_text(_str(r.get("position") or r.get("designation") or r.get("role") or "—")),
            "subsidiary":    sub,
            "leave_type":    normalize_cbc_text(_str(r.get("leave_Details") or "—")),
            "start_date":    _date_str(r.get("commencement_date") or r.get("start_date") or r.get("date_leave_starts")),
            "end_date":      _date_str(r.get("date_leave_ends") or r.get("end_date")),
            "days_applied":  _int(r.get("no_days_applying_for", 0)),
            "days_entitled": _int(r.get("no_days_entitled_to", 0)),
            "days_taken":    _int(r.get("no_days_taken", 0)),
            "days_left":     _int(r.get("no_days_left", 0)),
            "status":        _str(r.get("status") or "Unknown").title(),
            "date_applied":  _date_str(r.get("createdAt")),
            "request_id":    str(r.get("_id", ""))[-8:],
        })

    total      = len(rows)
    approved   = sum(1 for r in rows if r["status"] == "Approved")
    pending    = sum(1 for r in rows if r["status"] == "Pending")
    rejected   = sum(1 for r in rows if r["status"].lower() == "rejected")
    total_days = sum(r["days_applied"] for r in rows)
    avg_days   = round(total_days / total, 1) if total else 0

    # Top staff by days
    from collections import defaultdict
    staff_days  = defaultdict(int)
    staff_count = defaultdict(int)
    for r in rows:
        staff_days[r["staff_name"]]  += r["days_applied"]
        staff_count[r["staff_name"]] += 1
    top_staff = max(staff_days, key=staff_days.get) if staff_days else "—"
    top_staff_days = staff_days.get(top_staff, 0)

    # Subsidiary analytics
    sub_requests = defaultdict(int)
    sub_days     = defaultdict(int)
    sub_staff    = defaultdict(set)
    for r in rows:
        s = r["subsidiary"]
        sub_requests[s] += 1
        sub_days[s]     += r["days_applied"]
        sub_staff[s].add(r["staff_name"])

    total_days_all = sum(sub_days.values()) or 1
    sub_analytics = []
    for s in sorted(sub_requests.keys()):
        staff_count_s = len(sub_staff[s]) or 1
        sub_analytics.append({
            "subsidiary":    s,
            "requests":      sub_requests[s],
            "total_days":    sub_days[s],
            "avg_days":      round(sub_days[s] / staff_count_s, 1),
            "utilization_pct": round(sub_days[s] / total_days_all * 100, 1),
        })
    sub_analytics.sort(key=lambda x: x["total_days"], reverse=True)
    for rank, item in enumerate(sub_analytics, 1):
        item["rank"] = rank

    top_sub = sub_analytics[0]["subsidiary"] if sub_analytics else "—"
...
LEAVE_KPI_TEMPLATE = """<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>Leave KPI Report – IGEZ</title>
<script src="https://cdn.plot.ly/plotly-2.27.0.min.js"></script>
<style>
*{box-sizing:border-box;margin:0;padding:0}
body{font-family:'Segoe UI',Arial,sans-serif;background:linear-gradient(180deg,#f8fbff 0%,#edf5ff 100%);color:#0b2447;min-height:100vh}
.header{background:linear-gradient(135deg,#061b3a 0%,#082f63 55%,#1155cc 100%);color:white;padding:20px 28px;display:flex;align-items:center;justify-content:space-between;box-shadow:0 12px 30px rgba(8,47,99,.22)}
.header h1{font-size:20px;font-weight:800}
.header .sub{font-size:12px;opacity:.8;margin-top:3px}
.hbtn{background:rgba(255,255,255,.15);border:1px solid rgba(255,255,255,.4);color:white;padding:8px 16px;border-radius:8px;cursor:pointer;font-size:12px;font-weight:700;text-decoration:none;white-space:nowrap}
.hbtn:hover{background:rgba(255,255,255,.28)}
.hbtn.accent{background:rgba(17,85,204,.5);border-color:rgba(255,255,255,.6)}
.wrap{max-width:1400px;margin:0 auto;padding:20px 24px}
.section-title{font-size:15px;font-weight:800;color:#082f63;margin:24px 0 12px;padding-left:10px;border-left:4px solid #1155cc}
/* Filter bar */
.filter-bar{background:rgba(255,255,255,.8);border:1px solid rgba(143,195,255,.5);border-radius:12px;padding:14px 18px;display:flex;flex-wrap:wrap;gap:12px;align-items:flex-end;box-shadow:0 10px 28px rgba(8,47,99,.10);margin-bottom:8px}
.filter-bar label{display:grid;gap:4px;font-size:11px;color:#244a78;font-weight:800;min-width:120px}
.filter-bar select,.filter-bar input{border:1px solid #c7ddf6;border-radius:7px;padding:8px 10px;font:inherit;color:#082f63;background:#fff;width:100%}
.filter-bar button{border:0;border-radius:8px;padding:9px 18px;background:#1155cc;color:white;font-weight:700;cursor:pointer;align-self:flex-end}
/* KPI cards */
.kpi-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:14px}
.kpi{background:linear-gradient(145deg,rgba(255,255,255,.92),rgba(231,243,255,.66));border:1px solid rgba(143,195,255,.52);border-radius:12px;padding:16px 18px;box-shadow:0 18px 42px rgba(8,47,99,.12);border-top:4px solid var(--accent,#1155cc)}
.kpi .label{font-size:11px;color:#4b6f9b;text-transform:uppercase;letter-spacing:.5px;font-weight:800}
.kpi .value{font-size:28px;font-weight:900;color:#061b3a;margin:6px 0 2px}
.kpi .sub{font-size:11px;color:#5d7899}
/* Charts */
.chart-grid-2{display:grid;grid-template-columns:1fr 1fr;gap:16px}
.chart-grid-3{display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px}
.chart-card{background:linear-gradient(145deg,rgba(255,255,255,.92),rgba(239,247,255,.72));border:1px solid rgba(143,195,255,.48);border-radius:12px;padding:16px;box-shadow:0 18px 42px rgba(8,47,99,.10)}
.chart-card.full{grid-column:1/-1}
/* Sub table */
.sub-table-wrap{overflow-x:auto;border-radius:10px;box-shadow:0 10px 28px rgba(8,47,99,.08)}
table.sub-table{width:100%;border-collapse:collapse;font-size:12px;background:white;border-radius:10px;overflow:hidden}
table.sub-table th{background:#082f63;color:white;padding:10px 12px;text-align:left;font-size:11px;text-transform:uppercase;white-space:nowrap}
table.sub-table td{padding:9px 12px;border-bottom:1px solid #edf5ff;color:#244a78;vertical-align:top}
table.sub-table tr:hover td{background:#f3f8ff}
/* Staff table */
.table-tools{display:flex;flex-wrap:wrap;gap:10px;align-items:center;margin-bottom:10px}
.table-tools input{flex:1;min-width:200px;border:1px solid #c7ddf6;border-radius:8px;padding:8px 12px;font:inherit;color:#082f63}
.table-tools select{border:1px solid #c7ddf6;border-radius:8px;padding:8px 10px;font:inherit;color:#082f63;background:#fff}
.table-tools .btn{border:0;border-radius:8px;padding:8px 14px;background:#1155cc;color:white;font-size:12px;font-weight:700;cursor:pointer}
.pill{display:inline-block;padding:3px 9px;border-radius:999px;font-size:11px;font-weight:800;background:#dbeafe;color:#1155cc}
.pill.approved{background:#d1fae5;color:#065f46}
.pill.pending{background:#fef3c7;color:#92400e}
.pill.rejected{background:#fee2e2;color:#991b1b}
.pager{display:flex;gap:12px;align-items:center;justify-content:center;margin-top:14px;font-size:13px;color:#456489}
.pager .btn{border:0;border-radius:8px;padding:7px 14px;background:#1155cc;color:white;font-size:12px;font-weight:700;cursor:pointer}
.export-bar{display:flex;gap:10px;margin-bottom:12px;flex-wrap:wrap}
.export-btn{border:1px solid #c7ddf6;border-radius:8px;padding:8px 16px;background:white;color:#082f63;font-size:12px;font-weight:700;cursor:pointer;text-decoration:none;display:inline-block}
.export-btn:hover{background:#f3f8ff}
.rank-badge{display:inline-flex;align-items:center;justify-content:center;width:24px;height:24px;border-radius:50%;background:#1155cc;color:white;font-size:11px;font-weight:900}
...
<div class="header">
  <div style="display:flex;align-items:center;gap:14px">
    <img src="/static/cbc_logo.png" alt="CBC" style="height:44px;width:44px;border-radius:8px;object-fit:contain">
    <div>
      <div style="font-size:10px;font-weight:600;letter-spacing:2px;opacity:.7;text-transform:uppercase">CBC · Powered by IGEZ</div>
      <h1>Leave KPI Report</h1>
      <div class="sub">CBC Paperless · Live data from MongoDB · {{ now }}</div>
    </div>
  </div>
  <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
    <a href="/leave-kpi-export/csv?{{ filter_query }}" class="hbtn accent">&#x2B07; CSV</a>
    <a href="/leave-kpi-export/excel?{{ filter_query }}" class="hbtn accent">&#x2B07; Excel</a>
    <a href="/leave-kpi-export/pdf?{{ filter_query }}" class="hbtn accent">&#x2B07; PDF</a>
    <a href="/page/overview" class="hbtn">&#x2190; Dashboard</a>
  </div>
</div>

<div class="wrap">
  <!-- Filters -->
  <form class="filter-bar" method="get" action="/page/leave-kpi">
    <label>Subsidiary
      <select name="subsidiary">
        <option value="All">All subsidiaries</option>
        {% for s in sub_options %}<option value="{{ s }}" {% if filters.subsidiary == s %}selected{% endif %}>{{ s }}</option>{% endfor %}
      </select>
    </label>
    <label>Leave Type
      <select name="leave_type">
        <option value="">All types</option>
        {% for t in leave_types %}<option value="{{ t }}" {% if filters.leave_type == t %}selected{% endif %}>{{ t }}</option>{% endfor %}
      </select>
    </label>
    <label>Status
      <select name="status">
    ...
    <label>From <input type="date" name="start" value="{{ filters.start or '' }}"></label>
    <label>To   <input type="date" name="end"   value="{{ filters.end or '' }}"></label>
    <button type="submit">Apply</button>
    <a href="/page/leave-kpi" class="hbtn" style="background:#f3f8ff;color:#082f63;border:1px solid #c7ddf6;align-self:flex-end">Clear</a>
  </form>

  <!-- KPI Cards -->
  <div class="section-title">Key Performance Indicators</div>
  <div class="kpi-grid">
    <div class="kpi" style="--accent:#1155cc"><div class="label">Total Requests</div><div class="value">{{ d.total }}</div><div class="sub">All leave applications</div></div>
    <div class="kpi" style="--accent:#065f46"><div class="label">Approved</div><div class="value">{{ d.approved }}</div><div class="sub">{{ '%.0f'|format(d.approved/d.total*100 if d.total else 0) }}% approval rate</div></div>
    <div class="kpi" style="--accent:#92400e"><div class="label">Pending</div><div class="value">{{ d.pending }}</div><div class="sub">Awaiting approval</div></div>
    <div class="kpi" style="--accent:#991b1b"><div class="label">Rejected</div><div class="value">{{ d.rejected }}</div><div class="sub">{{ '%.0f'|format(d.rejected/d.total*100 if d.total else 0) }}% rejection rate</div></div>
    <div class="kpi" style="--accent:#0b3a75"><div class="label">Total Days Taken</div><div class="value">{{ d.total_days }}</div><div class="sub">Across all requests</div></div>
    <div class="kpi" style="--accent:#4c93ff"><div class="label">Avg Days / Staff</div><div class="value">{{ d.avg_days }}</div><div class="sub">Average per application</div></div>
    <div class="kpi" style="--accent:#1155cc"><div class="label">Highest Usage Staff</div><div class="value" style="font-size:16px">{{ d.top_staff }}</div><div class="sub">{{ d.top_staff_days }} days applied</div></div>
    <div class="kpi" style="--accent:#082f63"><div class="label">Top Subsidiary</div><div class="value" style="font-size:16px">{{ d.top_sub }}</div><div class="sub">Highest leave volume</div></div>
  </div>
"""

LEAVE_KPI_TEMPLATE += """
  <!-- Subsidiary Comparison Charts -->
  <div class="section-title">Subsidiary Comparative Analytics</div>
  <div class="chart-grid-2">
    <div class="chart-card">{{ charts.sub_requests | safe }}</div>
    <div class="chart-card">{{ charts.sub_days | safe }}</div>
  </div>
  <div class="chart-grid-3" style="margin-top:16px">
    <div class="chart-card">{{ charts.sub_avg | safe }}</div>
    <div class="chart-card">{{ charts.sub_pct | safe }}</div>
    <div class="chart-card">{{ charts.leave_type_pie | safe }}</div>
  </div>
  <div class="chart-grid-2" style="margin-top:16px">
    <div class="chart-card">{{ charts.status_pie | safe }}</div>
    <div class="chart-card">{{ charts.monthly_trend | safe }}</div>
  </div>

  <!-- Subsidiary Ranking Table -->
  <div class="section-title">Subsidiary Ranking — Leave Utilisation</div>
  <div class="sub-table-wrap">
    <table class="sub-table">
      <thead><tr>
        <th>Rank</th><th>Subsidiary</th><th>Requests</th>
        <th>Total Days</th><th>Avg Days/Staff</th>
        <th>% of Total</th>
      </tr></thead>
      <tbody>
      {% for row in d.sub_analytics %}
      <tr>
        ...
        <td>
          <div style="display:flex;align-items:center;gap:8px">
            <div style="flex:1;height:8px;background:#dbeafe;border-radius:4px;overflow:hidden">
              <div style="height:100%;width:{{ row.utilization_pct }}%;background:linear-gradient(90deg,#1155cc,#4c93ff);border-radius:4px"></div>
            </div>
            <span style="font-size:11px;font-weight:800;color:#082f63;white-space:nowrap">{{ row.utilization_pct }}%</span>
          </div>
        </td>
      </tr>
      {% else %}
      <tr><td colspan="6" style="text-align:center;color:#5d7899;padding:20px">No data for selected filters.</td></tr>
      {% endfor %}
      </tbody>
    </table>
  </div>
  ...
  <!-- Staff Detail Table -->
  <div class="section-title">Staff Leave Detail</div>
  <div class="table-tools">
    <input id="staffSearch" placeholder="Search by name, subsidiary, department, leave type…" oninput="filterStaffTable()">
    <select id="subFilter" onchange="filterStaffTable()">
      <option value="">All subsidiaries</option>
      {% for s in sub_options %}<option value="{{ s }}">{{ s }}</option>{% endfor %}
    </select>
    <select id="typeFilter" onchange="filterStaffTable()">
      <option value="">All leave types</option>
      {% for t in leave_types %}<option value="{{ t }}">{{ t }}</option>{% endfor %}
    </select>
    <select id="statusFilter2" onchange="filterStaffTable()">
      <option value="">All statuses</option>
  ...
  <div class="sub-table-wrap">
    <table class="sub-table" id="staffTable">
      <thead><tr>
        <th>#</th><th>Staff Name</th><th>Employee ID</th>
        <th>Department</th><th>Position</th><th>Subsidiary</th>
        <th>Leave Type</th><th>Start Date</th><th>End Date</th>
        <th>Days Applied</th><th>Entitled</th><th>Taken</th><th>Balance</th>
        <th>Status</th><th>Date Applied</th>
      </tr></thead>
      <tbody id="staffTbody">
      {% for row in d.rows %}
      <tr class="staff-row"
          data-name="{{ row.staff_name | lower }}"
          data-sub="{{ row.subsidiary | lower }}"
          data-type="{{ row.leave_type | lower }}"
          data-status="{{ row.status | lower }}"
          data-dept="{{ row.department | lower }}">
        <td>{{ loop.index }}</td>
        ...
        <td>{{ row.end_date }}</td>
        <td style="font-weight:800;color:#082f63">{{ row.days_applied }}</td>
        <td>{{ row.days_entitled if row.days_entitled else '—' }}</td>
        <td>{{ row.days_taken if row.days_taken else '—' }}</td>
        <td style="color:{% if row.days_left < 3 %}#991b1b{% else %}#065f46{% endif %};font-weight:800">{{ row.days_left if row.days_left else '—' }}</td>
        <td><span class="pill {{ row.status | lower }}">{{ row.status }}</span></td>
        <td>{{ row.date_applied }}</td>
      </tr>
      {% else %}
      <tr><td colspan="15" style="text-align:center;color:#5d7899;padding:20px">No leave records found.</td></tr>
      {% endfor %}
      </tbody>
    </table>
  </div>
  <div class="pager">
    <button class="btn" id="prevPage" onclick="changePage(-1)">Prev</button>
    <span id="pageInfo" style="color:#456489;font-size:13px"></span>
...
function getAllRows() {
  return Array.from(document.querySelectorAll('#staffTbody .staff-row'));
}

function filterStaffTable() {
  const q    = document.getElementById('staffSearch').value.toLowerCase();
  const sub  = document.getElementById('subFilter').value.toLowerCase();
  const type = document.getElementById('typeFilter').value.toLowerCase();
  const stat = document.getElementById('statusFilter2').value.toLowerCase();
  visibleRows = getAllRows().filter(r => {
    const name = r.dataset.name || '';
    const rs   = r.dataset.sub  || '';
    const rt   = r.dataset.type || '';
    const rst  = r.dataset.status || '';
    const dept = r.dataset.dept || '';
    const text = r.innerText.toLowerCase();
    return (!q || text.includes(q) || name.includes(q) || dept.includes(q))
        && (!sub  || rs.includes(sub))
        && (!type || rt.includes(type))
        && (!stat || rst.includes(stat));
  });
  currentPage = 1;
  renderPage();
}
...
function exportTableCSV() {
  const headers = Array.from(document.querySelectorAll('#staffTable thead th'))
  ...
  a.download = 'leave-kpi-staff.csv';
  a.click();
}
...
def leave_kpi_page():
    ...
    try:
        ...
        subsidiary   = freq.args.get("subsidiary", "All")
        leave_type_f = freq.args.get("leave_type", "")
        status_f     = freq.args.get("status", "")
        start_str    = freq.args.get("start", "")
        end_str      = freq.args.get("end", "")
        ...
        d = build_leave_kpi_data(db, sub_map, filters)

        # Apply additional in-memory filters (leave_type, status)
        rows = d["rows"]
        if leave_type_f:
            rows = [r for r in rows if r["leave_type"].lower() == leave_type_f.lower()]
        if status_f:
            rows = [r for r in rows if r["status"].lower() == status_f.lower()]
        d["rows"] = rows

        # Re-compute totals after secondary filter
        d["total"]      = len(rows)
        d["approved"]   = sum(1 for r in rows if r["status"] == "Approved")
        d["pending"]    = sum(1 for r in rows if r["status"] == "Pending")
        d["rejected"]   = sum(1 for r in rows if r["status"].lower() == "rejected")
        d["total_days"] = sum(r["days_applied"] for r in rows)
        d["avg_days"]   = round(d["total_days"] / d["total"], 1) if d["total"] else 0

        # Subsidiary options + leave types for filter dropdowns
        sub_options  = sorted({r["subsidiary"] for r in d["rows"]} | {r["subsidiary"] for r in build_leave_kpi_data(db, sub_map, {})["rows"]} - {""})
        leave_types  = sorted({r["leave_type"]  for r in build_leave_kpi_data(db, sub_map, {})["rows"]} - {""} - {"—"})

        # Build charts
        sa = d["sub_analytics"]
        sub_labels   = [x["subsidiary"] for x in sa]
        sub_req_vals = [x["requests"]   for x in sa]
        sub_day_vals = [x["total_days"] for x in sa]
        sub_avg_vals = [x["avg_days"]   for x in sa]
        sub_pct_vals = [x["utilization_pct"] for x in sa]

        # monthly trend from filtered rows
        from collections import defaultdict
        monthly = defaultdict(int)
        for r in rows:
            # parse date_applied back to month label
            m = r["date_applied"][:8] if len(r["date_applied"]) >= 7 else r["date_applied"]
            monthly[m] += r["days_applied"]
        monthly_sorted = dict(sorted(monthly.items()))

        # status counts for pie
        status_counts = {"Approved": d["approved"], "Pending": d["pending"], "Rejected": d["rejected"]}
        ltype_counts  = defaultdict(int)
        for r in rows:
            ltype_counts[r["leave_type"]] += 1

        charts = {}
        charts["sub_requests"]   = bar(sub_labels, sub_req_vals, "Leave Requests by Subsidiary (count)", "#1155cc")
        charts["sub_days"]       = bar(sub_labels, sub_day_vals, "Total Days Applied by Subsidiary",     "#0b3a75")
        charts["sub_avg"]        = bar(sub_labels, sub_avg_vals, "Average Days per Staff by Subsidiary", "#4c93ff")
        charts["sub_pct"]        = bar(sub_labels, sub_pct_vals, "% Contribution to Total Leave Days",   "#1d6ff2")
        charts["status_pie"]     = pie(list(status_counts.keys()), list(status_counts.values()), "Leave Request Status")
        charts["leave_type_pie"] = pie(list(ltype_counts.keys()),  list(ltype_counts.values()),  "Leave Types Distribution")
        charts["monthly_trend"]  = line(list(monthly_sorted.keys()), list(monthly_sorted.values()), "Monthly Leave Days Applied", "#1155cc")

        filter_query = freq.query_string.decode("utf-8")
        ...
        return render_template_string(
            ERROR_TEMPLATE,
            error_message=f"{exc}<br><pre>{traceback.format_exc()}</pre>",
            now=now,
        ), 500
...
@app.route("/leave-kpi-export/<fmt>")
def leave_kpi_export(fmt):
    now_str = app_now().strftime("%d %b %Y, %H:%M")
    try:
        db = get_db()
        sub_map = get_subsidiary_map(db)
        from flask import request as freq
        ...
        HEADERS = ["#", "Staff Name", "Employee ID", "Department", "Position",
                   "Subsidiary", "Leave Type", "Start Date", "End Date",
                   "Days Applied", "Days Entitled", "Days Taken", "Balance",
                   "Status", "Date Applied"]

        def row_values(i, r):
            return [i+1, r["staff_name"], r["employee_id"], r["department"],
                    r["position"], r["subsidiary"], r["leave_type"],
                    r["start_date"], r["end_date"], r["days_applied"],
                    r["days_entitled"], r["days_taken"], r["days_left"],
                    r["status"], r["date_applied"]]

        date_tag = app_now().strftime("%Y-%m-%d")
        ...
        if fmt == "csv":
            ...
            for i, r in enumerate(rows):
                w.writerow(row_values(i, r))
            return app.response_class(
                buf.getvalue(),
                mimetype="text/csv",
                headers={"Content-Disposition": f"attachment;filename=leave-kpi-{date_tag}.csv"},
            )
        ...
        # ── Excel ─────────────────────────────────────────────────────────────
            ...
            try:
                ...
                ws["A1"] = f"Leave KPI Report — Generated {now_str}"
                ws["A1"].font = Font(bold=True, size=13, color="FFFFFF")
                ws["A1"].fill = PatternFill("solid", fgColor="082f63")
                ws["A1"].alignment = Alignment(horizontal="center")
                ws.row_dimensions[1].height = 22

                # Header row
                hdr_fill = PatternFill("solid", fgColor="1155CC")
                ...
                for i, r in enumerate(rows):
                    ...
                    sc = ws.cell(row=i+3, column=status_col)
                ...
                for item in [
                    ...
                    ("Avg Days/Staff", d["avg_days"]),
                    ("Top Staff",      f"{d['top_staff']} ({d['top_staff_days']} days)"),
                    ("Top Subsidiary", d["top_sub"]),
                ]:
                    ws2.append(item)
                ...
                return send_file(buf, mimetype="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                 as_attachment=True,
                                 download_name=f"leave-kpi-{date_tag}.xlsx")
            except ImportError:
                return "openpyxl is required for Excel export. Install it with: pip install openpyxl", 400
        ...
        # ── PDF ──────────────────────────────────────────────────────────────
            ...
            try:
                ...
                title_s   = ParagraphStyle("T", parent=styles["Title"], fontSize=16,
                                           textColor=colors.HexColor("#082f63"))
                section_s = ParagraphStyle("S", parent=styles["Heading2"], fontSize=11,
                                           textColor=colors.HexColor("#082f63"), spaceBefore=10)
                body_s    = ParagraphStyle("B", parent=styles["BodyText"], fontSize=8, leading=11)

                story = [
                    Paragraph("Leave KPI Report", title_s),
                    Paragraph(f"Generated: {escape(now_str)} | Subsidiary: {escape(subsidiary or 'All')}", body_s),
                    Spacer(1, 8),
                ]

                # KPI summary table
                kpi_data = [
                    ["Total Requests", str(d["total"]),   "Approved",   str(d["approved"])],
                    ["Pending",        str(d["pending"]), "Rejected",   str(d["rejected"])],
                    ["Total Days",     str(d["total_days"]), "Avg Days", str(d["avg_days"])],
                    ["Top Staff",      escape(d["top_staff"]), "Top Sub", escape(d["top_sub"])],
                ]
                kpi_tbl = Table(kpi_data, colWidths=[100, 80, 100, 80])
                kpi_tbl.setStyle(TableStyle([
                    ("BACKGROUND", (0,0), (-1,-1), colors.HexColor("#f3f8ff")),
                    ("BACKGROUND", (0,0), (0,-1), colors.HexColor("#082f63")),
                    ("BACKGROUND", (2,0), (2,-1), colors.HexColor("#082f63")),
                    ("TEXTCOLOR",  (0,0), (0,-1), colors.white),
                ...
                story.extend([kpi_tbl, Spacer(1, 10)])

                # Subsidiary ranking
                story.append(Paragraph("Subsidiary Ranking", section_s))
                sub_data = [["Rank","Subsidiary","Requests","Total Days","Avg Days","% Share"]]
                for sa_row in d["sub_analytics"]:
                    sub_data.append([
                        str(sa_row["rank"]), escape(sa_row["subsidiary"]),
                        str(sa_row["requests"]), str(sa_row["total_days"]),
                        str(sa_row["avg_days"]), f"{sa_row['utilization_pct']}%",
                    ])
                sub_tbl = Table(sub_data, colWidths=[35, 130, 60, 70, 70, 60])
                sub_tbl.setStyle(TableStyle([
                    ("BACKGROUND", (0,0), (-1,0), colors.HexColor("#0b3a75")),
                    ("TEXTCOLOR",  (0,0), (-1,0), colors.white),
                    ("ROWBACKGROUNDS", (0,1), (-1,-1), [colors.white, colors.HexColor("#edf5ff")]),
                    ("GRID", (0,0), (-1,-1), 0.3, colors.HexColor("#c7ddf6")),
                    ("FONTSIZE", (0,0), (-1,-1), 8),
                    ("PADDING",  (0,0), (-1,-1), 5),
                ]))
                story.extend([sub_tbl, Spacer(1, 10)])

                # Staff detail table (paginated by reportlab)
                story.append(Paragraph("Staff Leave Detail", section_s))
                pdf_headers = ["#","Name","Dept","Subsidiary","Leave Type",
                               "Start","End","Days","Entitled","Balance","Status"]
                col_w = [20, 90, 70, 80, 70, 52, 52, 30, 40, 40, 50]
                staff_data = [pdf_headers]
                for i, r in enumerate(rows[:500]):
                    staff_data.append([
                        str(i+1),
                        escape(r["staff_name"][:22]),
                        escape(r["department"][:16]),
                        escape(r["subsidiary"][:18]),
                        escape(r["leave_type"][:16]),
                        r["start_date"], r["end_date"],
                        str(r["days_applied"]),
                        str(r["days_entitled"]) if r["days_entitled"] else "—",
                        str(r["days_left"]) if r["days_left"] else "—",
                        escape(r["status"]),
                    ])
                staff_tbl = Table(staff_data, colWidths=col_w, repeatRows=1)
                staff_tbl.setStyle(TableStyle([
                    ("BACKGROUND", (0,0), (-1,0), colors.HexColor("#1155cc")),
                    ("TEXTCOLOR",  (0,0), (-1,0), colors.white),
                    ("ROWBACKGROUNDS", (0,1), (-1,-1), [colors.white, colors.HexColor("#edf5ff")]),
                    ("GRID", (0,0), (-1,-1), 0.2, colors.HexColor("#c7ddf6")),
                    ("FONTSIZE", (0,0), (-1,-1), 7),
                    ("PADDING",  (0,0), (-1,-1), 4),
                    ("VALIGN",   (0,0), (-1,-1), "TOP"),
                ]))
                ...
                return send_file(buf, mimetype="application/pdf",
                                 as_attachment=True,
                                 download_name=f"leave-kpi-{date_tag}.pdf")
            except ImportError as e:
                return f"reportlab is required for PDF export: {e}", 400

        return "Unknown export format", 400

    except Exception as exc:
        import traceback
        return f"Export error: {exc}<br><pre>{traceback.format_exc()}</pre>", 500
